...
const routeManifestPath = path.resolve("worker/cms-managed-routes.ts");
...
function assertRequiredSingleton(documents, expectedId, label) {
  assertSingleton(documents, expectedId, label);
  if (documents.length !== 1) {
    throw new Error(
      `[sanity-sync] ${label} must be published before CMS-managed public routes can be enabled.`,
    );
  }
}
...
function collectManagedRoutes(content) {
  const routes = [];
  for (const document of content.pages ?? []) routes.push(normalizePublicPath(document.path));
  for (const document of content.legalDocuments ?? []) routes.push(normalizePublicPath(document.path));
  for (const document of content.hubArticles ?? []) routes.push(`/hub/${document.slug}`);
  for (const document of content.stories ?? []) routes.push(`/stories/${document.slug}`);
  for (const document of content.jobs ?? []) routes.push(`/careers/${document.slug}`);
  assertUnique(routes, "CMS managed public route");
...
function validateSnapshot(content) {
  ...
  const managedRoutes = collectManagedRoutes(content);
  if (managedRoutes.length > 0) {
    // Every native CMS route renders the global header/footer from the same
    // bilingual snapshot. Fail closed instead of publishing a half-configured
    // page with mismatched global chrome.
    assertRequiredSingleton(content.siteSettings, "siteSettings", "Site settings");
    assertRequiredSingleton(content.navigation, "mainNavigation", "Navigation");
  }
...
const managedRoutes = validateSnapshot(content);
...
await fs.writeFile(
  routeManifestPath,
  `// Generated by tools/sync-sanity-site.mjs. Do not edit by hand.\nexport const cmsManagedRoutes = new Set<string>(${JSON.stringify(managedRoutes, null, 2)});\n`,
  "utf8",
);
...
console.log(
  `[sanity-sync] Published snapshot validated with one Content Lake query: ${Object.entries(counts)
    ...
    .join(", ")}; managedRoutes=${managedRoutes.length}.`,
);
